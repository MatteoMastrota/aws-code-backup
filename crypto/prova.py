...
if __name__ == '__main__':
    ...
    while maxSize <=20:
        i = i0
        rho = []
        lossProbabilities[maxSize]=[]
        wait[maxSize]=[]
        realWaitDict[maxSize]=[]
        cust[maxSize]=[]
        serverBusy[maxSize]=[]
        arrivi[maxSize]=[]
        custQueue[maxSize]=[]        
        ...
        while i < 10:
            ...
            lossProbabilities[maxSize].append(loss)
            wait[maxSize].append(waiting)
            realWaitDict[maxSize].append(realWait)
            cust[maxSize].append(avgCust)
            serverBusy[maxSize].append(serverB)
            arrivi[maxSize].append(arr)
            custQueue[maxSize].append(avgCustQueue)
    ...
    # Loss probabilities
    for key in lossProbabilities.keys():
        # print(lossProbabilities[key])
        plt.plot(rho,lossProbabilities[key],label='B='+str(key+1),color = colors[key+1])
    plt.ylabel('Loss probability')
    plt.xlabel('Load')
    plt.title('M/M/1')
    ...
    for key in wait.keys():
        plt.plot(rho,wait[key],label='B='+str(key+1),color = colors[key+1])
    plt.ylabel('Average Waiting in queue')
    plt.xlabel('Load')
    plt.title('M/M/1')
    plt.legend(loc='lower right')
    plt.show()
    ...
    # realWaiting
    for key in realWaitDict.keys():
        plt.plot(rho,realWaitDict[key],label='B='+str(key+1),color = colors[key+1])
    plt.ylabel('Average Waiting in queue of packets entering queue')
    plt.xlabel('Load')
    plt.title('M/M/1')
    plt.legend(loc='lower right')
    plt.show()
    ...
    for key in cust.keys():
        plt.plot(rho,cust[key],label='B='+str(key+1),color = colors[key+1])
    plt.ylabel('Average number of user E[N]')
    plt.xlabel('Load')
    plt.title('M/M/1')
    plt.legend(loc='lower right')
    plt.show()
    ...
    # serverBusy
    for key in serverBusy.keys():
        plt.plot(rho,serverBusy[key],label='B='+str(key+1),color = colors[key+1])
    plt.ylabel('Utilization Server')
    plt.xlabel('Load')
    plt.title('M/M/1')
    plt.legend(loc='lower right')
    plt.show()
    ...
    # arrivi
    for key in arrivi.keys():
        plt.plot(rho,arrivi[key],label='B='+str(key+1),color = colors[key+1])
    ...
    # custQueue
    for key in custQueue.keys():
        plt.plot(rho,custQueue[key],label='B='+str(key+1),color = colors[key+1])
    plt.ylabel('Average number of customer in queue')
    plt.xlabel('Load')
    plt.title('M/M/1')
    plt.legend(loc='lower right')
    plt.show()