...
def encryptLazy(ptext):
    """Encript without permutation (4 rounds)"""
    state = addKey(intToVec((w[0] << 8) + w[1]), intToVec(ptext))
    # first round
...
def encryptVeryLazy(ptext):
    """Encript without permutation (4 rounds)"""
    state = addKey(intToVec((w[4] << 8) + w[5]), intToVec(ptext))
    # first round
    state = computeRoundLazy(w[4],w[5],state)
    # second round
    state = computeRoundLazy(w[4],w[5],state)
...
if __name__ == '__main__':
    ...
    #ciphertext1 = encryptLazy(plaintext)
    #plaintext1 = decryptLazy(ciphertext1)
    
    ciphertext1 = encryptVeryLazy(plaintext)
    plaintext1 = decryptVeryLazy(ciphertext1)
  
    try:
        assert plaintext == plaintext1
    except AssertionError:
        print("Encryption error")
        print(plaintext, ciphertext1, plaintext1)
        print(decryptLazy(ciphertext1),plaintext)
        sys.exit(1)
    print("Test ok!")
    
    # ciphertext1 = encrypt_4(plaintext)
    # plaintext1 = decrypt_4(ciphertext1)
    # try:
    #     assert plaintext == plaintext1
    # except AssertionError:
    #     print("Encryption error")
    #     print(encrypt_4(plaintext), ciphertext)
    #     sys.exit(1)
    # print("Test ok!")
    
    # ciphertext1 = encrypt_3(plaintext)
    # plaintext1 = decrypt_3(ciphertext1)
    # try:
    #     assert plaintext == plaintext1
    # except AssertionError:
    #     print("Encryption error")
    #     print(encrypt_3(plaintext), ciphertext)
    #     sys.exit(1)
    # print("Test ok!")
    ...
    error = 1 << random.randrange(16) #bitwise left shift di un numero [0,16] di posizioni
    plaintext2 = plaintext ^ error #bitwise XOR
    assert hamming(plaintext, plaintext2) == 1
    
    record_hamming = np.zeros(1000)
    for i in range(1000):
        plaintext = random.getrandbits(16)
        key = random.getrandbits(16)
        keyExp(key)
        ciphertext = encryptVeryLazy(plaintext)
        #ciphertext = encryptLazy(plaintext)
        #ciphertext = encrypt_4(plaintext)
        # ciphertext = encrypt(plaintext)
        
        error = 1 << random.randrange(16) #bitwise left shift di un numero [0,16] di posizioni
        plaintext2 = plaintext ^ error #bitwise XOR
        ciphertext2 = encryptVeryLazy(plaintext2)
        #ciphertext2 = encryptLazy(plaintext2)
        #ciphertext2 = encrypt_3(plaintext2)
        #ciphertext2 = encrypt(plaintext2)
        record_hamming[i] = hamming(ciphertext,ciphertext2)
    print("Average number of bit changed in the ciphertext (plaintext): " + str(np.average(record_hamming)) )   
    ...
    for i in range(1000):
        ...
        ciphertext = encryptVeryLazy(plaintext)
        #ciphertext = encryptLazy(plaintext)
        #ciphertext = encrypt_3(plaintext)
        #ciphertext = encrypt(plaintext)
        
        error = 1 << random.randrange(16) #bitwise left shift di un numero [0,16] di posizioni
        key2 = key ^ error #bitwise XOR
        keyExp(key2)
        ciphertext2 = encryptVeryLazy(plaintext)
        #ciphertext2 = encryptLazy(plaintext)
        #ciphertext2 = encrypt_3(plaintext)
        #ciphertext2 = encrypt(plaintext)
        record_hamming[i] = hamming(ciphertext,ciphertext2)
    print("Average number of bit changed in the ciphertext (key): " + str(np.average(record_hamming)) )   
    ...
    print(encrypt_foo(known_plain))
    state = sub4NibList(sBox,intToVec(known_plain))
    state = shiftRow(state)
    key = addKey(state,intToVec(known_cipher))
    keyExp(vecToInt(key))
    print(encrypt_foo(known_plain))
    print(encrypt_foo(known_plain) == known_cipher)
    
    byteArray = bytearray(encryption)
    print(byteArray.pop())
    decrypt = []
    while byteArray:
        if len(byteArray)%2 != 0:
            oddCharacter = byteArray.pop()
            ciphertext = (oddCharacter<< 8)
        else:    
            leastSignificative = byteArray.pop()
            mostSignificative = byteArray.pop()
            ciphertext = (mostSignificative <<8) + leastSignificative 
        decrypt_bin = decrypt_foo(ciphertext)
        first_char = (decrypt_bin & 0xff00) >> 8
        second_char = (decrypt_bin & 0x00ff)
        
        decrypt.append(chr(second_char))
        decrypt.append(chr(first_char))
    decrypt.reverse() 
    s=''
    print(s.join(decrypt))
