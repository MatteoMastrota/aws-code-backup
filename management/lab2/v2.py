...
def arrival(time, FES, queue):
    ...
    batteryCharge = random.randint(0,50)/100*CAPACITY   #Battery remaining of arriving client
    # print(batteryCharge)
    ...
    if len(queue) > 0 :
        ...
        while len(waitingLine) > 0:
            ...
            if Customer[1] < time:
                ...
                FES.put((time + fullChargeTime, "fullcharge"))  # If not expired serve the customer (give the charged battery and retrieve the uncharged one, total doesn't change)
                return
...
while time < SIM_TIME:
    ...
    STATISTIC.append([station.nReady,station.nCharge])
...
readyOverTime = [x[0] for x in STATISTIC]
chargeOverTime = [x[1] for x in STATISTIC]
print(len(readyOverTime))
# x= []
# for i in range(0,len(readyOverTime)):
    # x.append(i)
# print(len(x))
plt.plot(readyOverTime[100:300])
plt.show()
