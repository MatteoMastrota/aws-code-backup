...
noRenewable=0
for element in totalEnergyUsed.keys():
    noRenewable+=pow(totalEnergyUsed[element],-6)*summerPrice[element%24]
print("Spesa no rinnovabile ",noRenewable)

withRenewable=0
for element in totalEnergyUsed.keys():
    if summerEnergy[element%24]:
        withRenewable+=(pow(totalEnergyUsed[element],-6)-pow(summerEnergy[element%24],-6))*summerPrice[element%24]
    else:
        withRenewable+=pow(totalEnergyUsed[element],-6)*summerPrice[element%24]
